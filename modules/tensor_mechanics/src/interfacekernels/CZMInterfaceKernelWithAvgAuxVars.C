...
#include "CZMInterfaceKernelWithAvgAuxVars.h"

registerMooseObject("TensorMechanicsApp", CZMInterfaceKernelWithAvgAuxVars);

template <>
InputParameters
validParams<CZMInterfaceKernelWithAvgAuxVars>()
{
  InputParameters params = validParams<CZMInterfaceKernel>();
  ...
  //                                       " disp_index == 1, ==> Y"
  //                                       " disp_index == 2, ==> Z");
  params.addCoupledVar("var_0_name",
                       "Name of the variable representing the first coupled average MP");
...
CZMInterfaceKernelWithAvgAuxVars::CZMInterfaceKernelWithAvgAuxVars(
    const InputParameters & parameters)
  : CZMInterfaceKernel(parameters),
    _var0(coupledValue("var_0_name")),
    _var0_var(coupled("var_0_name")),
    _var0_neighbor(coupledNeighborValue("var_0_name")),
    _var0_neighbor_var(coupled("var_0_name")),
    _mp_vars_id(getMaterialProperty<std::map<unsigned int /*var_id*/, unsigned int /*mp_index*/>>(
...

{
  ...
  {
    mooseError("In order to use  CZMInterfaceKernelWithAvgAuxVars ,"
               " you must specify a boundary where it will live.");
  }
}

// Real
// CZMInterfaceKernelWithAvgAuxVars::computeQpResidual(Moose::DGResidualType type)
// {
//
...
// Real
// CZMInterfaceKernelWithAvgAuxVars::computeQpJacobian(Moose::DGJacobianType type)
// {
...
Real
CZMInterfaceKernelWithAvgAuxVars::computeQpOffDiagJacobian(Moose::DGJacobianType type,
                                                           unsigned int jvar)
{

  if (jvar != _disp_1_var && jvar != _disp_2_var && jvar != _disp_1_neighbor_var &&
      jvar != _disp_2_neighbor_var && jvar != _var0_var && jvar != _var0_neighbor_var)
  {
  ...
  Real jac_coeff = 1;
  std::cout << "computing JVAR: " << jvar << " _var0_var: " << _var0_var
            << " _var0_neighbor_var: " << _var0_neighbor_var << std::endl;
  ...
  else if (jvar == _var0_var || jvar == _var0_neighbor_var)
  {
    std::cout << "computing aux var jacobian: ";
    auto value = _mp_vars_id[_qp].find(jvar);
    if (value != _mp_vars_id[_qp].end())
    {

      OffDiagIndex = value->second;
      std::cout << " OffDiagIndex " << OffDiagIndex << std::endl;
      jac_coeff = 0.5;
    }
    else
      mooseError("CZMInterfaceKernelWithAvgAuxVars::cannot find the correct material property "
                 "given this varaible id");
  }
  else
  {
  ...
  switch (type)
  {
    ...
    // master variables
    case Moose::ElementElement:
      jac *= -_test[_i][_qp] * _phi[_j][_qp] * (-1) * jac_coeff;
      break;

    case Moose::ElementNeighbor:
      jac *= -_test[_i][_qp] * _phi_neighbor[_j][_qp] * (1) * jac_coeff;
      break;

    case Moose::NeighborElement:
      jac *= _test_neighbor[_i][_qp] * _phi[_j][_qp] * (-1) * jac_coeff;
      break;

    case Moose::NeighborNeighbor:
      jac *= _test_neighbor[_i][_qp] * _phi_neighbor[_j][_qp] * (1) * jac_coeff;
      break;
