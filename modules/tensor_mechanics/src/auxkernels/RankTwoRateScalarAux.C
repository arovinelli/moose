...
#include "RankTwoRateScalarAux.h"
#include "RankTwoScalarTools.h"

registerMooseObject("TensorMechanicsApp", RankTwoRateScalarAux);

template <>
InputParameters
validParams<RankTwoRateScalarAux>()
{
...
RankTwoRateScalarAux::RankTwoRateScalarAux(const InputParameters & parameters)
  : NodalPatchRecovery(parameters),
    _tensor(getMaterialProperty<RankTwoTensor>("rank_two_tensor")),
    _tensor_old(getMaterialPropertyOld<RankTwoTensor>("rank_two_tensor")),
...
Real
RankTwoRateScalarAux::computeValue()
{
  unsigned int qp = _qp;
  if (_has_selected_qp)
  {
    if (_selected_qp >= _q_point.size())
    {
      ...
      mooseError("RankTwoRateScalarAux.  selected_qp specified as ",
                 _selected_qp,
                 " but there are only ",
                 _q_point.size(),
  ...
  RankTwoTensor tensor_rate = (_tensor[qp] - _tensor_old[qp]) / _dt;

  return RankTwoScalarTools::getQuantity(
      tensor_rate, _scalar_type, _point1, _point2, _q_point[qp], _input_direction);
}
