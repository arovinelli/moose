...
#include "CZMCopenetrationPenalty.h"
#include "Material.h"
#include "MooseError.h"

registerMooseObject("TensorMechanicsApp", CZMCopenetrationPenalty);
template <>
InputParameters
validParams<CZMCopenetrationPenalty>()
{
  ...
  params.addRequiredParam<Real>(
      "copenetration_penalty_stiffness",
      "The value of the copenetration penalty stiffness, usually one or two order of magnitude "
      "higher of the bulk material elastic sitffeness");
  params.addClassDescription("UO implementing copenetration penalty");
  return params;
}

CZMCopenetrationPenalty::CZMCopenetrationPenalty(const InputParameters & parameters)
  : CZMTractionSeparationUOBase(parameters),
    ...
    _copenetration_penalty_stiffness(getParam<Real>("copenetration_penalty_stiffness"))

{
}

RealVectorValue
CZMCopenetrationPenalty::computeTractionLocal(unsigned int qp) const
{
  RealVectorValue TractionLocal;

  if (_weighted_displacement_jump[qp][0] < 0)
  {
    for (unsigned int i = 0; i < 3; i++)
      TractionLocal(i) = _copenetration_penalty_stiffness * _weighted_displacement_jump[qp][i];
  }
  else
    mooseError(
        "CZMCopenetrationPenalty:: should not be called if _weighted_displacement_jump >= 0");
  return TractionLocal;
}

RankTwoTensor
CZMCopenetrationPenalty::computeTractionSpatialDerivativeLocal(unsigned int qp) const
{
      ...
      for (unsigned int j = 0; j < 3; j++)
        if (i == j)
          TractionDerivativeLocal(i, j) =
              _copenetration_penalty_stiffness * _displacement_jump_weights[qp][i];
        else
          TractionDerivativeLocal(i, j) = 0;
  }
  else
    mooseError(
        "CZMCopenetrationPenalty:: should not be called if if _weighted_displacement_jump >= 0");
