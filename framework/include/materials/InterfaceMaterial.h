 ...
 */
class InterfaceMaterial : public MooseObject,
...
public:
  ...
  /**
   * A method for (re)computing the properties of a InterfaceMaterial.
   *
   * This is intended to be called from other objects, by first calling
   * MaterialPropertyInterface::getMaterial and then calling this method on the InterfaceMaterial
   * object returned.
   */
...
protected:
  ...
  /**
   * Copies dual number values from ADMaterials into Real property values for
   * InterfaceMaterial<->ADMaterial interoperability.
   */
  ...
  /// InterfaceMaterial::computeProperties() without looking up the ids from
...
InterfaceMaterial::getMaterialPropertyByName(const std::string & prop_name)
{
  ...
  registerPropName(prop_name, true, InterfaceMaterial::CURRENT);
...
{
  registerPropName(prop_name, true, InterfaceMaterial::OLD);
...
{
  registerPropName(prop_name, true, InterfaceMaterial::OLDER);
  return MaterialPropertyInterface::getMaterialPropertyOlderByName<T>(prop_name);
}

template <typename T>
MaterialProperty<T> &
InterfaceMaterial::declareProperty(const std::string & prop_name)
{
  registerPropName(prop_name, false, InterfaceMaterial::CURRENT);
  return _material_data->declareProperty<T>(prop_name);
}

template <typename T>
MaterialProperty<T> &
InterfaceMaterial::declarePropertyOld(const std::string & prop_name)
{
  mooseDoOnce(
  ...
  registerPropName(prop_name, false, InterfaceMaterial::OLD);
  return _material_data->declarePropertyOld<T>(prop_name);
}

template <typename T>
MaterialProperty<T> &
InterfaceMaterial::declarePropertyOlder(const std::string & prop_name)
{
  mooseDoOnce(
  ...
  registerPropName(prop_name, false, InterfaceMaterial::OLDER);
  return _material_data->declarePropertyOlder<T>(prop_name);
}

template <typename T>
const MaterialProperty<T> &
InterfaceMaterial::getZeroMaterialProperty(const std::string & prop_name)
{
  checkExecutionStage();
