...
protected:
   ...
   * MaterialBase<->ADMaterial interoperability.
   */
...
MaterialBase::getADMaterialProperty(const std::string & name)
{
  // Check if the supplied parameter is a valid imput parameter key
  std::string prop_name = deducePropertyName(name);
...
template <typename T>
const ADMaterialPropertyObject<T> &
MaterialBase::getADMaterialPropertyByName(const std::string & prop_name)
{
  checkExecutionStage();
