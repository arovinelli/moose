...
validParams<MaterialBase>()
{
  ...
  params.addParam<bool>("compute",
                        true,
                        "When false, MOOSE will not call compute methods on this MaterialBase. "
  ...
  params.addParam<std::vector<std::string>>(
      "output_properties",
      "List of MaterialBase properties, from this MaterialBase, to output (outputs "
...
MaterialBase::MaterialBase(const InputParameters & parameters)
  : MooseObject(parameters),
    ...
    // for MaterialBase objects the hide lists are handled by MaterialOutputAction
...
MaterialBase::initStatefulProperties(unsigned int n_points)
{
  ...
  for (auto & prop : _supplied_props)
    if (_material_data->getMaterialPropertyStorage().isStatefulProp(prop) &&
      ...
      mooseError(std::string("MaterialBase \"") + name() +
...
MaterialBase::checkStatefulSanity() const
{
  for (const auto & it : _props_to_flags)
    if (static_cast<int>(it.second) % 2 == 0) // Only Stateful properties declared!
      mooseError(
          "MaterialBase '", name(), "' requests undefined stateful property '", it.first, "'");
}

void
MaterialBase::registerPropName(std::string prop_name,
                               bool is_get,
                               MaterialBase::Prop_State state,
                               bool is_declared_ad)
{
  if (!is_get)
  {
  ...
  // Store MaterialBase properties for block ids
  for (const auto & block_id : blockIDs())
    _fe_problem.storeSubdomainMatPropName(block_id, prop_name);

  // Store MaterialBase properties for the boundary ids
...
{
  mooseError("computeSubdomainQpProperties in MaterialBase '", name(), "' needs to be implemented");
}
...
MaterialBase::computeProperties()
{
  ...
  // just the ones for this MaterialBase.
  MaterialProperties & props = _material_data->props();

  // If this MaterialBase has the _constant_on_elem flag set, we take the
...
MaterialBase::resetQpProperties()
{
  if (!_compute)
    mooseDoOnce(
        mooseWarning("You disabled the computation of this (",
                     name(),
                     ") MaterialBase by MOOSE, but have not overridden the 'resetQpProperties' "
                     "method, this can lead to unintended values being set for MaterialBase "
                     "property values."));
}
...
MaterialBase::checkExecutionStage()
{
  if (_fe_problem.startedInitialSetup())
    mooseError(
        "MaterialBase properties must be retrieved during MaterialBase object construction to "
