...
validParams<InterfaceMaterial>()
{
  ...
  params.addParam<bool>(
      ...
      "The user must call computeProperties() after retrieving the InterfaceMaterial "
  ...
  params.registerBase("InterfaceMaterial");
...
InterfaceMaterial::InterfaceMaterial(const InputParameters & parameters)
  : MooseObject(parameters),
    ...
    // for InterfaceMaterial objects the hide lists are handled by MaterialOutputAction
...
void
InterfaceMaterial::initStatefulProperties(unsigned int n_points)
{
  ...
  for (auto & prop : _supplied_props)
    if (_material_data->getMaterialPropertyStorage().isStatefulProp(prop) &&
      ...
      mooseError(std::string("InterfaceMaterial \"") + name() +
...
void
InterfaceMaterial::initQpStatefulProperties()
{
  _overrides_init_stateful_props = false;
}

void
InterfaceMaterial::checkStatefulSanity() const
{
  for (const auto & it : _props_to_flags)
    if (static_cast<int>(it.second) % 2 == 0) // Only Stateful properties declared!
      mooseError(
          "InterfaceMaterial '", name(), "' requests undefined stateful property '", it.first, "'");
}

void
InterfaceMaterial::registerPropName(std::string prop_name,
                                    bool is_get,
                                    InterfaceMaterial::Prop_State state,
                                    bool is_declared_ad)
{
  if (!is_get)
  {
...
void
InterfaceMaterial::subdomainSetup()
{
...
void
InterfaceMaterial::computeSubdomainProperties()
{
  mooseError(
      "computeSubdomainQpProperties in InterfaceMaterial '", name(), "' needs to be implemented");
}
...
InterfaceMaterial::computeProperties()
{
  ...
  // just the ones for this InterfaceMaterial.
  MaterialProperties & props = _material_data->props();

  // If this InterfaceMaterial has the _constant_on_elem flag set, we take the
...
void
InterfaceMaterial::copyDualNumbersToValues()
{
...
void
InterfaceMaterial::computeQpProperties()
{
}

void
InterfaceMaterial::resetProperties()
{
  for (_qp = 0; _qp < _qrule->n_points(); ++_qp)
    resetQpProperties();
}
...
void
InterfaceMaterial::resetQpProperties()
{
  if (!_compute)
...
void
InterfaceMaterial::computePropertiesAtQp(unsigned int qp)
{
  _qp = qp;
  computeQpProperties();
}
...
void
InterfaceMaterial::checkExecutionStage()
{
  if (_fe_problem.startedInitialSetup())
    mooseError(
        "InterfaceMaterial properties must be retrieved during material object construction to "
