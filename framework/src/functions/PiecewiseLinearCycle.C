...
registerMooseObject("MooseApp", PiecewiseLinearCycle);

template <>
InputParameters
validParams<PiecewiseLinearCycle>()
{
  InputParameters params = validParams<PiecewiseLinear>();
  params.addRequiredParam<FunctionName>("cycle_fraction_func", "The cycle fraction function name");
...
Real
PiecewiseLinearCycle::value(Real t, const Point & p)
{
...
Real
PiecewiseLinearCycle::timeDerivative(Real t, const Point & p)
{
  Real func_value;
  if (_has_axis)
  {
  ...
  {
    func_value = _linear_interp->sampleDerivative(_cycle_fraction_func.value(t, p));
  }
  return _scale_factor * func_value;
}

Real
PiecewiseLinearCycle::integral()
{
  return _scale_factor * _linear_interp->integrate();
}

Real
PiecewiseLinearCycle::average()
{
  return integral() /
