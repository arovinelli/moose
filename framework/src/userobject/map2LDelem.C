...
registerMooseObject("MooseApp", map2LDelem);
...
map2LDelem::initialize()
{
  ...
  const std::map<dof_id_type, std::vector<dof_id_type>> & n_2_elem_map = _mesh.nodeToElemMap();
  ...
  for (unsigned int i = 0; i < elem_side_bid.size(); i++)
  {
    ...
    if (boundaryList.find(std::get<2>(elem_side_bid[i])) != boundaryList.end())
    {
      ...
      for (unsigned int k = 0; k < nodes_on_boundary_side.size(); k++)
        nodes_on_current_side.insert(elem_ptr->node_id(nodes_on_boundary_side[k]));

      // find global node id of current local node
      unsigned int global_node_id = elem_ptr->node_id(nodes_on_boundary_side[0]);
          ...
          n_2_elem_map.end()) // if there are elements (this might be removed)
      {
        ...
        for (auto elem_id : list_of_connected_elem->second)
        {
          ...
          if (neighbor_ptr->dim() == (elem_ptr->dim() - 1))
          {
            ...
            // we loop over all its node to check that all of them belong to the current side
          ...
          if (is_LD_neighbor) // found a neighbor we need to aadd the element to the map
          {
        ...
        if (!is_LD_neighbor) // we couldn't find a neighbor
          mooseError("can't find LD neighbor for element " + std::to_string(elem_ptr->id()) +
                     " on side " + std::to_string(std::get<1>(elem_side_bid[i])));
      }
      else
