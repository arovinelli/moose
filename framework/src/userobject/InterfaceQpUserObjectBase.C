...
InterfaceQpUserObjectBase::validParams()
{
  ...
  params.addParam<bool>(
      "compute_rate", false, "if true, compute the rate the selected interfacevalue");
  return params;
}
...
InterfaceQpUserObjectBase::initialSetup()
{
  ...
  for (unsigned int i = 0; i < elem_side_bid.size(); i++)
  {
    ...
    if (boundaryList.find(std::get<2>(elem_side_bid[i])) != boundaryList.end())
    {
      ...
      _JxW_values[elem_side_pair] = var_values;
    }
  }
}
...
InterfaceQpUserObjectBase::execute()
{
  ...
  if (it != _map_values.end())
  {
    ...
    auto & jxw = _JxW_values[std::make_pair(_current_elem->id(), _current_side)];
    ...
    for (unsigned int qp = 0; qp < _qrule->n_points(); ++qp)
    {
      ...
      vec[qp] = computeInterfaceValueType(computeValueMaster(qp), computeValueSlave(qp));
      jxw[qp] = _JxW[qp];
    }
  }
  else
                                               ...
                                               const unsigned int side) const
{
  ...
  auto weights = _JxW_values.find(std::make_pair(elem, side));
  if (weights == _JxW_values.end())
    mooseError("getSideAverageValue: can't find the given qp");
