...
#include "InterfaceUOMaterial.h"
#include "MooseMesh.h"

registerMooseObject("MooseTestApp", InterfaceUOMaterial);

template <>
InputParameters
validParams<InterfaceUOMaterial>()
{
...
InterfaceUOMaterial::computeQpProperties()
{
  ...
  _boundary_property[_qp] = (_material_property_average[_qp] + _variable_jump[_qp]);
}
